...
const Simpson38Rule = ({navigation}) => {
  const handleSubmit = e => {
    e.preventDefault();
    const getData = async () => {
      try {
        await axios
          .post('http://localhost:8081/ODE/simpson38', {input}) //For Flask server
          .then(res => {
            if (res.data.message) {
              setErrorMessage(res.data.message);
            } else {
              navigation.navigate('Simpson 3/8 Rule SOL', {
                data: res.data,
              });
  ...
  return (
    <>
      ...
      <View style={styles.container}>
        <Text style={styles.title}>Simpson 3/8 Rule</Text>
